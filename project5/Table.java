...
public class Table< K , E >
{
    ...
    public static int extractInt(String line){
        ...
        for(int i = 0; i < line.length() && cutoff == 0; i++){
            if(line.charAt(i) == ' '){
                cutoff = i;
            }
        }

        String isolatedInt = line.substring(cutoff+1, line.length());
        int returnKey = Integer.parseInt(isolatedInt);
    ...
    public static String extractName(String line){
        ...
        String name = line.substring(0, cutoff);
